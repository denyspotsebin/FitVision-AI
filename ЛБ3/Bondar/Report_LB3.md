...
Для тестування було обрано модуль `FitVisionSystemManager` (та модель `FitnessData`), який відповідає за бізнес-логіку системи (валідація фітнес-показників користувача та управління системою сповіщень). Код містить нетривіальну логіку (перевірки діапазонів, цикли `foreach`, перехоплення винятків та резервну відправку Email при збої Push-сповіщень). Завдяки використанню інтерфейсу `INotificationService` модуль повністю ізольований для незалежного модульного тестування.
