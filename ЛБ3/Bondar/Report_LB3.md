...
### 7. Бонусне завдання (Автоматизація тестування)
Було успішно налаштовано CI/CD пайплайн за допомогою GitHub Actions. Створено конфігураційний файл `.github/workflows/tests.yml`. Тепер при кожному оновленні коду (push) в репозиторій автоматично розгортається віртуальне середовище Ubuntu та запускається перевірка всіх тестів за допомогою `dotnet test`.
