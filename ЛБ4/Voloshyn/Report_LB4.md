...
## 7. Бонусне завдання: Використання AI-асистента (GitHub Copilot Chat)

Для виконання бонусного завдання було використано AI-асистента (Copilot Chat) як допоміжний інструмент під час проведення Code Review коду іншого учасника команди (Арсена). Метою було отримати пропозицію щодо оптимізації підозрілого фрагмента коду, який відповідав за видалення історії користувача.

**Запит (Prompt):**
> Проаналізуй цей код і запропонуй варіанти рефакторингу. Як можна зробити його більш ефективним?

**Аналізований фрагмент коду (код напарника):**
```csharp
public int ClearUserHistory(string userId)
{
    int deletedCount = 0;
    var itemsToRemove = new List<HistoryRecord>();
...
![Відповідь Copilot - частина 1](./images/bonus1.png)

![Відповідь Copilot - частина 2](./images/bonus2.png)

**Висновок:**
Пропозиція AI-асистента виявилася максимально влучною. Вона підтвердила мої власні підозри щодо неефективності використання двох циклів `foreach` для видалення елементів (зайві алокації пам'яті). Відповідну рекомендацію щодо оптимізації цього фрагмента було додано як коментар до Pull Request напарника під час Code Review.